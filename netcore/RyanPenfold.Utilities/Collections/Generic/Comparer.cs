// TODO: Find substitute for System.Collections.Comparer
/*
// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Comparer.cs" company="Inspire IT Ltd">
...
namespace RyanPenfold.Utilities.Collections.Generic
{
    ...
    public class Comparer<T> : IComparer<T>
    {
        ...
        public Comparer(string sortField, string sortDirection = "ASC")
        {
            ...
            var sortFieldIsValid = typeof(T).GetTypeInfo().GetProperties().Where(property => (property != null)).Any(property => property.Name == sortField)
                || typeof(T).GetTypeInfo().GetFields().Where(field => (field != null)).Any(field => field.Name == sortField);
            ...
            switch (sortFieldIsValid)
            {
                case true:
                    ...
                    break;
                case false:
                    throw new Exception($"Property or field with name \"{sortField}\" not found on type \"{typeof(T).Name}\".");
            }
        ...
        {
            var result = 0;
            var property = x.GetType().GetTypeInfo().GetProperty(this.sortField);
            var field = x.GetType().GetTypeInfo().GetField(this.sortField);

            if (property == null && field == null)
            {
                throw new Exception($"Property or field with name \"{this.sortField}\" not found in type \"{typeof(T).Name}\".");
            }
