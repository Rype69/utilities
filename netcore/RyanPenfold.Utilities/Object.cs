// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Object.cs" company="Inspire IT Ltd">
...
namespace RyanPenfold.Utilities
{
    ...
    public static class Object
    {
        ...
        public static object Clone<T>(this object source)
        {
            ...
            {
                result = System.Reflection.Assembly.Load(new System.Reflection.AssemblyName(assemblyName)).CreateInstance(typeName);
            }

            // Fields
            foreach (var field in source.GetType().GetTypeInfo().GetFields())
            {
                if (field == null)
                {
                }
            ...
            // Properties
            foreach (var prop in
                source.GetType().GetTypeInfo().GetProperties().Where(prop => (prop != null) && prop.CanWrite))
            {
                try
                {
                    result?.GetType().InvokeMember(
                        ...
                        null,
                        result,
                        new[] { source.GetType().GetTypeInfo().GetProperty(prop.Name).GetValue(source, null) });
                }
                catch (System.MissingFieldException)
                {
                }
        ...
        */
        ...
        /// </param>
        public static void CopyFrom<T>(this object destination, object source)
        {
            ...
            // Copy all properties
            foreach (var property in typeof(T).GetTypeInfo().GetProperties().Where(p => p.CanRead && p.CanWrite))
            {
        ...
        public static object GetMemberValue(this object obj, string memberName)
        {
            ...
            var propertyInfo = obj.GetType().GetTypeInfo().GetProperty(memberName);
        ...
        */
        ...
        public static string ToString<T>(T obj)
        {
            ...
            var properties = typeof(T).GetTypeInfo().GetProperties().Where(p => p != null).ToList();
            ...
            for (var propertyId = 0; propertyId < properties.Count; propertyId++)
            {
                ...
                // TODO: Check for DBNull if (value == null || value == System.DBNull.Value)
                if (value == null)
                {
                    continue;
                }
