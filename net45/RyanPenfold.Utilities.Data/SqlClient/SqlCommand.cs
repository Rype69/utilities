    ...
    public static class SqlCommand
    {
        ...
        public static object RunExecuteScalarFromFile(string fileName, string connectionString = null, string connectionStringName = null)
        {
            if (string.IsNullOrEmpty(connectionString) && string.IsNullOrEmpty(connectionStringName))
                throw new System.InvalidOperationException("Parameters \"parsedConnectionString\" and \"connectionStringName\" cannot both be null or empty.");
        ...
        {
            var parsedConnectionString = connectionString;

            if (string.IsNullOrEmpty(connectionString) && string.IsNullOrEmpty(connectionStringName))
                throw new System.InvalidOperationException("Parameters \"parsedConnectionString\" and \"connectionStringName\" cannot both be null or empty.");

            if (string.IsNullOrEmpty(connectionString) && !string.IsNullOrEmpty(connectionStringName))
                parsedConnectionString = GetConnectionString(connectionStringName);
            ...
            try
            {
                // Initialise the SQL command
                sqlCommand.CommandType = System.Data.CommandType.Text;
                sqlCommand.Connection = new System.Data.SqlClient.SqlConnection(parsedConnectionString);
                sqlCommand.CommandText = commandText;
        ...
        private static string GetConnectionString(string connectionStringName)
        {
            ...
            if (connectionStringSettings == null)
                throw new System.Configuration.ConfigurationErrorsException($"Connection string {connectionStringName} not found in the configuration settings.");
